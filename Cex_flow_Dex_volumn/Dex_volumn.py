...
def get_dex_volumn_by_time(chain_id, contract_address, time_from, time_to):
    ...
    while True:   
        ...
        #điều kiện để thoát vòng lặp: kết quả trả về có số bản ghi dưới 1000
        if len(response_json['results']) < 1000:
            break 
        time_from = readable_to_timestamp(response_json['results'][-1]['time'])
        result_list.extend(response_json['results'])
        
    #xử lí duplicate
    result_list = list(set(tuple(d.items()) for d in result_list))
    result_list = [dict(t) for t in result_list]
                
    #sum volumn based on time
    time_dict = {}
    for transaction in result_list:
        #format time theo giờ
        key = re.sub(r':\d{2}:\d{2}', ':00:00', transaction['time'])
        if key in time_dict:
...
# 7 day - 21/4
data_dex = get_dex_volumn('arbitrum','0x912CE59144191C1204E64559FE8253a0e49E6548', 1682640000)
# 2 day - 4/5
